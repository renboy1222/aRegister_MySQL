...
import com.aldrin.aregister.gui.dialog.au.JDialogRoomAU;
...
public class JDialogRoom extends javax.swing.JDialog implements MouseListener {
    ...
    public JDialogRoom(JFrameRegister jFrameRegister, boolean modal) {
  ...
  DefaultTableModel tableModel = new DefaultTableModel(new Object[]{"ID", "ROOM", "LOCATION"}, 0);

    private TableRowSorter<TableModel> sorter = new TableRowSorter<TableModel>(tableModel);

    private void setTable() {
        jTableRoom.setCellSelectionEnabled(true);
        jTableRoom = new JTable(tableModel);
        jScrollPane1.setViewportView(jTableRoom);
        jTableRoom.addMouseListener(this);
        jTableRoom.setRowSorter(sorter);
        TableColumn hide0 = jTableRoom.getColumnModel().getColumn(0);
        hide0.setMinWidth(0);
        hide0.setMaxWidth(0);
        hide0.setPreferredWidth(0);

        TableColumn[] column = new TableColumn[100];
        column[1] = jTableRoom.getColumnModel().getColumn(1);
        column[1].setPreferredWidth(100);

        column[2] = jTableRoom.getColumnModel().getColumn(2);
    ...
    private void selectCategory() {
        ...
        tableModel.setRowCount(0);
        for (Room r : roomList) {
            tableModel.addRow(new Object[]{r.getId(), r.getRoom(), r.getLocation()});
        }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == jTableRoom) {
            if (e.getButton() == MouseEvent.BUTTON1) {
                if (e.getClickCount() == 1) {
                    int row = jTableRoom.getSelectedRow();
                    if (row != -1) {
                        Long userId = Long.parseLong(jTableRoom.getValueAt(row, 0).toString());
                        String rooml = jTableRoom.getValueAt(row, 1).toString();
                        String location = jTableRoom.getValueAt(row, 2).toString();
                        Room r = new Room();
                        r.setId(userId);
                        r.setRoom(rooml);
